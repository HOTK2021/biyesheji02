...
function selectUserInfo(url){
    layui.use('table', function(){
        ...
        table.render({
            ...
            // ,limit :10
            ,cols : [[
                {field: 'userId', title: '用户ID', minWidth:30, align:"center"},
                {field: 'eighteenId', title: '受刑地狱', minWidth:150, align:'center',templet: function (d) {
                        if (d.eighteenId==1){
                            return " 皮肤科";
                ...
                {field: 'inOrOutTime', title: '诊断时间', minWidth:150, align:'center'},
                ...
                {title: '操作', width:300, templet:'#TorturedListBar',align:"center"}
            ]]
        });
...
layui.use(['form','layer','laydate','table','laytpl'],function(){
    ...
    function early(earlyedit){
        var index = layui.layer.open({
            title : "普通id",
            type : 2,
            content : "/html/page/userpage/earlyRelease.html",
            success : function(layero, index){
                var body = layui.layer.getChildFrame('body', index);
                if(earlyedit){
                    body.find("#userId4").val(earlyedit.userId);
                    body.find("#earlyTime").val(earlyedit.imprisonmentTime);
                    form.render();
                }
                setTimeout(function(){
                    layui.layer.tips('点击此处返回文章列表', '.layui-layer-setwin .layui-layer-close', {
    ...
    // })
//列表操作
    table.on('tool(Tortured)', function(obj){
        var layEvent = obj.event,
            data = obj.data;
        if(layEvent === 'earlyedit'){ //编辑
            early(data);
            // $.ajax({
            //     type: "post",
