...
layui.use(['form','layer','upload','laydate',"address"],function(){
    ...
    function imprisonmentTime() {
        for(var i=0;i<=500;i++){
            $("#earlyImprisonmentTime").append("<option value='"+i+"'>"+i+"</option>");
        }
    };
    function hell() {
        $.ajax({
    ...
    //提交个人资料
    form.on("submit(early)",function(){
        var addImprisonmentTime=$("#earlyImprisonmentTime").val();
        // 将填写的用户信息存到session以便下次调取
        $.ajax({
            type:"post",
            dataType:"json",
            data:$("#earlyRelease").serialize(),
