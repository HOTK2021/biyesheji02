	...
	Tab.prototype.navBar = function(strData){
		...
		for(var i=0;i<data.length;i++){
			if(data[i].spread || data[i].spread == undefined){
				...
				ulHtml += '<li class="layui-nav-item" id="'+data[i].id+'"  style="pointer-events:none ">';
				// for (var k=0;k<roles.length;k++){
				// 	if (data[i].id==roles[k].menu_id){
				// 		alert("进入");
				// 		$("#1005").css("pointer-events", "");
				// 		//$("#"+data[i].id).css("pointer-events", "");
				// 	}
				// }
			}
			if(data[i].children != undefined && data[i].children.length > 0){
				...
				for(var j=0;j<data[i].children.length;j++){
					if(data[i].children[j].target == "_blank"){
						...
						ulHtml += '<dd><a data-url="'+data[i].children[j].href+'" name="'+data[i].children[j].id+'">';
					}
